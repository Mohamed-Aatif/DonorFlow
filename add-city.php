                        ...
                        if ($result->num_rows > 0) { // Check if data exists
                            ...
                            while ($row = $result->fetch_assoc()) { // Fetch rows and display in table
                                ...
                                <tr class='hover:bg-gray-50 cursor-pointer' id="<?php echo $row['city_id']; ?>">
                                    ...
                                    <td class='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500'><?php echo $row['city']; ?></td>
                                    <td class='px-6 py-4 whitespace-nowrap text-sm text-blue-600'>
                                        <?php if ($row['city_status'] == 1) { $status = "checked"; } else { $status = ""; } ?><div class="checkbox-wrapper-51">
                                            <input type="checkbox" id="id-<?php echo $row['city_id']; ?>" data-id="<?php echo $row['city_id']; ?>" onclick="sendToPHP(this)" <?php echo $status; ?>>
                                            <label class="toggle" for="id-<?php echo $row['city_id']; ?>" data-id="id-<?php echo $row['city_id']; ?>" onclick="toggleStatus(this)">
                                                <span>
