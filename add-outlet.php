...
if (isset($_POST['id']) && isset($_POST['status'])) {
    $id = $_POST['id'];
    $status = $_POST['status'];
    $sql = "UPDATE `outlet` SET outlet_status = ? WHERE outlet_id = ?";
...
<main>
    ...
    <div class="breadcrumb-container">
        <ul class="breadcrumbs">
            ...
            <li><a href="add-outlet.php">Outlet</a></li>
        </ul>
    </div>
        ...
        <div class="body">
            <form action="" method="post">
                <div class="form-control">
                    ...
                    <label for="outlet_city">Select City<span class="required">*</span></label>
                    <select id="outlet_city" name="outlet_city" autofocus>
                ...
                <div class="form-btn">
                    <button type="submit" name="outlet-submit" class="success-btn">Submit</button>
        ...
        <div class="body">
            ...
            <table class="min-w-full divide-y divide-gray-200" id="myTable">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                    ...
                    <?php
                    $sql = "SELECT * FROM `outlet` ORDER BY outlet_city ASC , outlet ASC"; // Query to fetch data
                    ...
                    if ($result->num_rows > 0) { // Check if data exists
                        ...
                        while ($row = $result->fetch_assoc()) { // Fetch rows and display in table
                            ...
                            <tr class='hover:bg-gray-50 cursor-pointer' id="<?php echo $row['outlet_id']; ?>">
                                <td class='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500'><?php echo $serial_no; ?></td>
                                <td class='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500'><?php echo $row['outlet_city']; ?></td>
                                <td class='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500'><?php echo $row['outlet']; ?></td>
                                <td class='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500'>10</td>
                                <td class='px-6 py-4 whitespace-nowrap text-sm text-blue-600'><?php if ($row['outlet_status'] == 1) { $status = "checked"; } else { $status = ""; } ?>
                                    <div class="checkbox-wrapper-51">
                                        <input type="checkbox" id="id-<?php echo $row['outlet_id']; ?>"
                                            data-id="<?php echo $row['outlet_id']; ?>" onclick="sendToPHP(this)" <?php echo $status; ?>>
                                        <label class="toggle" for="id-<?php echo $row['outlet_id']; ?>"
                                            data-id="id-<?php echo $row['outlet_id']; ?>" onclick="toggleStatus(this)">
                                            <span>
    ...
    <div class="container modal-container">
        ...
        <div class="body">
            <form id="editForm" method="post">
                <input type="text" id="outletid" name="outletid" hidden>
                <div class="form-control">
                    <label for="outletcityupdate">City</label>
                ...
                <div class="form-control">
                    ...
                    <input type="text" id="outletupdate" name="outletupdate" autocomplete="off" required>
                </div>
                <div class="form-btn">
                    <button type="submit" name="outlet-update" class="success-btn">Submit</button>
...
<div class="popup-overlay" id="popup">
    <div class="popup">
        ...
        <form method="post">
            <input type="text" id="deleteoutletid" name="deleteoutletid" hidden>
            <button class="confirm-btn" name="delete-outlet">Yes, Delete</button>
...
<script>
    ...
    const outletcityupdate = document.getElementById("outletcityupdate");
    const outletupdate = document.getElementById("outletupdate");
    ...
    editButtons.forEach(button => {
        button.addEventListener("click", function () {
            ...
            const outletcity = row.cells[1].innerText; // Extract the data from the row's cells
            const outlet = row.cells[2].innerText; // Extract the data from the row's cells
            const rowId = row.id; // Extract the data from <tr> id 

            outletcityupdate.value = outletcity; // Populate the form with the row's data
            outletupdate.value = outlet; // Populate the form with the row's data
            outletid.value = rowId; // Populate the form with the row's data
